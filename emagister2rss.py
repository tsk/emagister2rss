                        ...
                        if element.attrib['class'] == "app_wall_item_content":
                            ...
                            for b in bs:
                                if b.attrib['class']=="wallTit":
                                    ...
                                    idata.append(linkdeb.attrib['href'].encode("utf-8"))
    ...
    for link in links:
        try:
            if link.attrib['class'] == "titulo":
                ...
                rss_group((opener.open(group_link).read()).decode("latin-1"),group_name,group_link)
        except:
            pass
    #search for more groups pages
    for link in links:
        if link.text == ' siguiente >':
            rss_groups((opener.open("http://grupos.emagister.com"+link.attrib['href']).read()).decode("latin-1"),opener)
